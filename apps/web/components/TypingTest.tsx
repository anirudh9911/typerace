...
export default function TypingTest() {
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      // Finish immediately when last character is typed
  ...
  const correctCount = useMemo(() => {
    return charStates.filter((state) => state === 'correct').length;
  }, [charStates]);

  const incorrectCount = useMemo(() => {
    return charStates.filter((state) => state === 'incorrect').length;
  }, [charStates]);
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center gap-8 bg-neutral-900 px-6 text-white">
      <div className="flex gap-6 text-sm text-neutral-300">
        ...
        <div>
          Incorrect: <span className="text-white">{incorrectCount}</span>
        </div>
      </div>
      ...
      <div className="max-w-2xl text-2xl leading-relaxed">
        {text.split('').map((char, index) => {
          ...
          return (
            <span
              key={index}
              className={`${getCharacterClass(index)} ${isCaret ? 'underline decoration-white underline-offset-4 text-white' : ''}`}
            >
              {char}
            </span>
          );
